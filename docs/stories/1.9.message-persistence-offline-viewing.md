...
- [ ] **Task 1: Verify Firestore Offline Persistence Configuration** (AC: 1, 2)
  - [ ] Open `MessageAI/Data/Network/FirebaseService.swift` (created in Story 1.2)
  - [ ] Verify offline persistence is enabled:
    ...
    ```
  - [ ] Verify this configuration already exists from Story 1.2
  - [ ] No code changes needed - just verification

- [ ] **Task 2: Implement Network Monitoring with NWPathMonitor** (AC: 4, 6)
  - [ ] Create `MessageAI/Presentation/Utils/NetworkMonitor.swift`
  - [ ] Implement NetworkMonitor class:
    ...
    ```
  - [ ] This replaces placeholder offline detection from Stories 1.7 and 1.8

- [ ] **Task 3: Integrate NetworkMonitor into ViewModels** (AC: 4, 6)
  ...
  - [ ] Replace placeholder offline detection:
    ...
    ```
  - [ ] Repeat for `ChatViewModel` in Story 1.8 (if completed)
  - [ ] NetworkMonitor is dependency-injected with default parameter for easy testing

- [ ] **Task 4: Update DIContainer with NetworkMonitor** (AC: 4, 6)
  - [ ] Open `MessageAI/App/DIContainer.swift`
  - [ ] Add NetworkMonitor singleton:
    ```swift
...
- [ ] **Task 6: Verify Timestamp Formatting for Offline Context** (AC: 7)
  - [ ] Open `MessageAI/Presentation/ViewModels/Conversations/ConversationsListViewModel.swift`
  - [ ] Verify `formattedTimestamp()` method exists (from Story 1.7):
    ```swift
    ...
    ```
  - [ ] Verify similar timestamp formatting exists in ChatViewModel for message timestamps
  - [ ] Relative timestamps ("2 hours ago") work correctly offline since they're calculated from local Date()
  - [ ] No code changes needed if formatting already exists from previous stories

- [ ] **Task 7: Create Integration Tests for Offline Scenarios** (AC: 8, 9, 10)
  - [ ] Create `MessageAITests/Integration/OfflinePersistenceIntegrationTests.swift`
  - [ ] Mark class with `@MainActor` for async testing
  - [ ] Write integration tests:
    ...
    ```
  - [ ] Tests are skeletons with `XCTSkip` - full implementation deferred to Story 1.10
  - [ ] Purpose: Document test coverage requirements and planned test scenarios

- [ ] **Task 8: Manual Offline Testing** (AC: 3, 4, 5, 6, 8, 9, 10, 11)
  - [ ] **Setup Phase (Online):**
    - [ ] Build and run app on simulator
    - [ ] Sign in as test user
    - [ ] Create or open existing conversation
    - [ ] Send 10+ messages back and forth
    - [ ] Verify messages appear in conversations list
    - [ ] Note conversation ID and message count for verification
  
  - [ ] **Test Scenario 1: Go Offline While App Running**
    - [ ] With app in foreground, enable airplane mode (Cmd+Shift+H in simulator, swipe down, airplane mode)
    - [ ] ✅ Verify offline banner appears in conversations list: "You're offline"
    - [ ] ✅ Verify conversations list still shows all conversations
    - [ ] ✅ Tap conversation to open chat view
    - [ ] ✅ Verify offline banner appears in chat view: "You're offline. Messages will send when connected."
    - [ ] ✅ Verify all messages load from cache (no loading spinner)
    - [ ] ✅ Scroll through conversation history - verify smooth scrolling, no errors
    - [ ] ✅ Try sending a message - verify optimistic UI shows message with "Sending..." status
    - [ ] ✅ Return to conversations list - verify conversation preview updated with offline message
    - [ ] ✅ No crashes or Firestore errors in console
  
  - [ ] **Test Scenario 2: App Killed and Restarted Offline**
    - [ ] With airplane mode still on, kill app (swipe up from bottom)
    - [ ] Reopen app from simulator home screen
    - [ ] ✅ Verify app launches successfully (no crash)
    - [ ] ✅ Verify authentication state persisted (still logged in)
    - [ ] ✅ Verify offline banner appears immediately
    - [ ] ✅ Verify conversations list loads from cache (no loading spinner)
    - [ ] ✅ Open conversation - verify messages load from cache
    - [ ] ✅ Verify timestamp formatting shows relative times ("2 hours ago")
    - [ ] ✅ No network-related errors in console
  
  - [ ] **Test Scenario 3: Return Online**
    - [ ] Disable airplane mode
    - [ ] ✅ Verify offline banner disappears within 2 seconds
    - [ ] ✅ Verify queued messages sync (status changes from "Sending..." to "Sent")
    - [ ] ✅ Verify any new messages from other users appear (if test environment supports)
    - [ ] ✅ Verify conversations list updates with latest timestamps
    - [ ] ✅ No crashes during offline → online transition
  
  - [ ] **Test Scenario 4: Edge Cases**
    - [ ] Go offline, send 5 messages, kill app, restart app offline
    - [ ] ✅ Verify all 5 offline messages still queued and visible
    - [ ] Go back online
    - [ ] ✅ Verify all 5 messages sync successfully
    - [ ] Test with poor connection (3G simulation): Settings → Developer → Network Link Conditioner
    - [ ] ✅ Verify app handles slow network gracefully (no crashes, loading indicators)

- [ ] **Task 9: Update Unit Tests for NetworkMonitor** (AC: 4, 6)
  - [ ] Open `MessageAITests/Presentation/ViewModels/ConversationsListViewModelTests.swift`
  - [ ] Add test for offline detection:
    ```swift
    ...
    ```
  - [ ] Create `MessageAITests/Utils/MockNetworkMonitor.swift`:
    ```swift
    import Foundation
    import Combine
    ...
    ```
  - [ ] Update existing ViewModel tests to pass MockNetworkMonitor to avoid real network checks

- [ ] **Task 10: Document Offline Behavior in README** (AC: All)
  - [ ] Open `README.md`
  - [ ] Add section to "Features Implemented" or "Testing":
    ```markdown
    ### Offline Support
    ...
    **Testing Offline:**
    ...
    ```
  - [ ] Update "Running Tests" section to note integration tests require emulator (Story 1.10)

- [ ] **Task 11: Verify No Regression in Existing Features** (AC: 10)
  - [ ] Run all existing unit tests: `./scripts/quick-test.sh`
  - [ ] Verify all tests pass (AuthViewModel, ProfileSetupViewModel, ConversationsListViewModel)
  - [ ] Run build: `./scripts/build.sh`
  - [ ] Verify no build errors or warnings
  - [ ] Manual smoke test:
    - [ ] Authentication flow works (sign up, sign in, logout)
    - [ ] Profile setup works
    - [ ] Conversations list loads and updates in real-time
    - [ ] Chat view sends and receives messages in real-time (if Story 1.8 complete)
  - [ ] Ensure offline functionality doesn't break online functionality
